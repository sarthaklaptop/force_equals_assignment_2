...
export async function PUT(request: NextRequest) {
  ...
  if (!session || !(session.user as SessionUser)?.id) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  ...
  await prisma.$transaction([
    prisma.sellerAvailability.deleteMany({ where: { userId: (session.user as SessionUser).id } }),
    prisma.sellerAvailability.createMany({ data: parsed.data.slots.map(s => ({ ...s, userId: (session.user as SessionUser).id, active: true })) })
  ])
